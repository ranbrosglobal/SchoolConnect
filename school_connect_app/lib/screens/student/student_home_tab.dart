...
class _StudentHomeTabState extends ConsumerState<StudentHomeTab> {
  ...
  Future<void> _loadStats() async {
    try {
      ...
      final attendance = await service.getMyAttendanceSummary();
      final timetable = await service.getMyTimetable();
      // Count unique classes from timetable
      final classes = <String>{};
      for (final entry in timetable.timetable) {
        classes.add(entry.classId ?? entry.studentGroupName ?? '');
      }
      classes.removeWhere((c) => c.isEmpty);
      // Extract schedule
      ...
      for (final entry in timetable.timetable) {
        ...
        if (subject.isNotEmpty && seenSubjects.add(subject)) {
          schedule.add({
            ...
            'title': subject,
            'subtitle': entry.instructorName ?? '',
          });
        }
      }
      if (!mounted) return;
      setState(() {
        _classCount = classes.isEmpty ? timetable.timetable.length : classes.length;
        _assignmentCount = assignments.length;
        _attendancePct = attendance.overallPercentage;
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Text('${_attendancePct.toStringAsFixed(0)}%', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.green)),
                      const SizedBox(width: 8),
                      Text('Attendance', style: TextStyle(fontSize: 10, color: Colors.green.shade700)),
                    ],
                  ),
                ),
              ),
