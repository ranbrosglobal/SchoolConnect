...
class _TeacherManageStudentsScreenState
  ...
  Future<void> _addStudent() async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(sheetsServiceProvider).createStudent(
            name: data.name,
            email: data.email,
  ...
  Future<void> _editStudent(StudentModel student) async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(sheetsServiceProvider).updateStudent(
  ...
  Future<void> _removeStudent(StudentModel student) async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(sheetsServiceProvider).deleteStudent(student.id);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
