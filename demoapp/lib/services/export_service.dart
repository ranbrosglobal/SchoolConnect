...
class ExportService {
  ...
  /// Fills [data]'s school-profile fields from the local DB (best effort).
  Future<void> _fillSchool(AttendanceExportData data) async {
    try {
      final db = await LocalDb.instance;
      final rows = await db.query('school_profile', where: 'id = 1');
      ...
      data.schoolName = get('school_name', 'School Connect');
      data.schoolMotto = get('motto');
      data.schoolEmail = get('contact_email');
      data.schoolPhone = get('contact_number');
      data.schoolWebsite = get('website');
      data.schoolAddress = get('address');

      // Logo: only local asset paths are embedded (the demo is offline).
      final logo = get('logo');
      if (logo.startsWith('assets/')) {
        try {
          final bytes = await rootBundle.load(logo);
          data.schoolLogoBytes = bytes.buffer.asUint8List();
        } catch (_) {}
      }
    } catch (_) {}
  }
