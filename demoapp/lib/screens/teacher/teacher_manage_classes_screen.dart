...
class _TeacherManageClassesScreenState
  ...
  Future<void> _addClass() async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(demoApiServiceProvider).createStudentGroup(
            name: data.name,
  ...
  Future<void> _editClass(StudentGroupModel group) async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(demoApiServiceProvider).updateStudentGroup(
            groupId: group.id,
            name: data.name,
            program: data.program,
          );
  ...
  Future<void> _removeClass(StudentGroupModel group) async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(demoApiServiceProvider).deleteStudentGroup(group.id);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
...
class _SubjectManagementScreenState
  ...
  Future<void> _addSubject() async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(demoApiServiceProvider).createCourseSchedule(
            course: data.course,
